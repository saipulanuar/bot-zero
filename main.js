...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya W4RIA STORE Bot saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk memulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        buff = await getBuffer(
          `http://hadi-api.herokuapp.com/api/card/welcome?nama=${anu_user}&descriminator=${groupMembers.length
            ...
            mdata.subject
          )}&pp=${pp_user}&bg=https://i.ibb.co/5jZNVN6/Whats-App-Image-2021-12-23-at-21-21-28.jpg`
        )

        but = [
          { buttonId: 'add', buttonText: { displayText: 'Welcome Member Baru' }, type: 1 }
        ]
        sendButImage(mdata.id, wel, "©Created : W4RIA STORE", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        memeg = mdata.participants.length
        out = `𝐆𝐨𝐨𝐝𝐁𝐲𝐞 @${anu_user} 𝐒𝐞𝐦𝐨𝐠𝐚 𝐭𝐞𝐧𝐚𝐧𝐠 𝐝𝐢 𝐬𝐚𝐧𝐚`
        buff = await getBuffer(
        ...
        sendButImage(mdata.id, out, "©Created : W4RIA STORE", buff, but)
      }
